...
def get_coshow(contents):
    ...
    cut_content = jieba.lcut(clean_content) #cut in precise mode
    cut_content = list(filter(lambda x: x!=' ', cut_content)) #如果不是空白就放入list
    #建立新詞，並計算出現頻率
...
#concentrate all news, remove punctuation, cut it by jieba, and remove stop words.
...
#return 出來的詞最少2字，最多1000字，n: return 幾個
